...
{
    public class AddRoleCommandHandler : IRequestHandler<AddRoleCommand, MessageDTO>
    {
        ...
        {
            var roleName = EntityName.Create(request.Name);
            if (!roleName.IsSuccess)
            {
                return MessageDTO.Failure(roleName.Title, roleName.Errors, roleName.Message);
            }
            var existRole = await _roleRepository.ExistsAsync(x => x.Name.Value == roleName.Data.Value, cancellationToken);
            if (existRole)
            {
